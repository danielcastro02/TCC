                ...
                if ($this->logado->getId_usuario() == $chat->getIdRemetente()) {
                    echo "
                            ...
                            <div class='card-image'>
                                ...
                                <img class='materialboxed' id='" . $chat->getIdChat() . "' src='" . $chat->getCaminhoMedia() . "'>
                                <script>
                                var ponto;
                                var url_atual = window.location.href;
                                if(url_atual.indexOf('index.php')>-1){
                                    ponto = '';
                                }else{
                                    ponto = '.'
                                }
                                $('#" . $chat->getIdChat() . "').attr('src' , ponto +$('#" . $chat->getIdChat() . "').attr('src'))
                                </script>
                    ...
                    $pdo = conexao::getConexao();
                    $stmt = $pdo->prepare("update chat set visualizado = 1 where id_chat = :id_chat");
                    $stmt->bindValue(":id_chat", $chat->getIdChat());
                    $stmt->execute();
                    echo "
                            <div class='row'>
                ...
                if ($this->logado->getId_usuario() == $chat->getIdRemetente()) {
                    echo "
                            ...
                            <div style='width: 80%; margin-left: 20%'>
                                ...
                                <source  id='" . $chat->getIdChat() . "' src='" . $chat->getCaminhoMedia() . "' type='video/mp4'><source src=\"mov_bbb.flv\" type=\"video/flv\"> Browser Not Supporting
                                </video>
                                <input class='last_id' hidden value='" . $chat->getIdChat() . "'/>            
                                <span style='color: #5A5A5A; text-align: right; font-size: 9px; margin-left: 5px' >" . $hora->format('d/m H:i') . "</span>
                                <script>
...
//            echo "De um oi!";
            $chat = new Chat();
            $chat->setMensagem("De um oi!");
            $chat->setIdDestinatario($this->logado->getId_usuario());
            $chat->setIdRemetente($_GET['destinatario']);
            $chat->setDataEnvio("2000-00-00 00:00:00");
            $this->drowMessage($chat);
        }
    }
...
//        }
        ...
        switch ($extensao) {
            ...
            case '.svg':
                ...
                break;
            case '.gif':
                move_uploaded_file($_FILES['arquivo']['tmp_name'], __DIR__ . '/../Img/Chat/Media/' . $nome_imagem . '.gif');
                $extFinal = ".gif";
                break;
            case '.png':
    ...
    function selectListaContatos()
    {
        $con = new conexao();
        $pdo = $con->getConexao();
        $stmt = $pdo->prepare('select u.*  , count(ch.id_chat) as newMessages from usuario as u left outer join  chat as ch on ch.id_remetente = u.id_usuario and ch.id_destinatario = :id_destinatario and ch.visualizado = 0 and u.administrador > :adm group by u.id_usuario;');
        $stmt->bindValue(':id_destinatario', $this->logado->getId_usuario());
        switch ($this->logado->getAdministrador()) {
            case 2:
            case 1:
                $stmt->bindValue(":adm", -1);
                break;
            case 0:
                $stmt->bindValue(":adm", 0);
    ...
    function refreshBodyChat()
    {
        ...
        while ($linha = $stmt->fetch()) {
            ...
            if ($this->logado->getId_usuario() == $usuario->getId_usuario()) {

            } else {
                if ($this->logado->getAdministrador() == 0 and $usuario->getAdministrador() == 0) {

                } else {
                    echo "  <li class='hoverable vali waves-effect openChat collection-item' id_destinatario='" . $usuario->getId_usuario() . "'>
                                       
                                        <div class='fotoPerfil le' style='background-image: url(" . ($usuario->getIs_foto_url() == 1 ? $usuario->getFoto() : $pontos . $usuario->getFoto()) . ");
                                        float: left;
                                        margin: 0 8px 0 -12px;
